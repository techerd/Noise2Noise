...
"""
Code for 1 variable, 5 climate models, annual data
"""
...
class TrainDataset(Dataset):
    ...
    def __init__(self, variable, normalisation, train_models_list):
        ...
        self.root_dir = '/scratch/globc/techer/ia/data_norm/'+self.normalisation+'/'+self.variable+'/' # Path with normalized data for one normalization and one variable for all models
        self.train_models_list = train_models_list #Names of training models for one fold
            ...
            for mem_input in train_members_list[1:]: # Exclusion of the member 0 that is the ensemble mean
                for mem_output in train_members_list[1:]: # Exclusion of the member 0 that is the ensemble mean
                    if mem_input==mem_output # When we have the same members in one pair we use the ensemble mean 
                        pairs_of_members.append([model, train_members_list[0], mem_input]) # Save the name of the model and the members file names
                    else:
    ...
    def __getitem__(self, item):
        ...
        input_member = np.load(input_name, allow_pickle=True)[3:,:,:]  # Erase the first three years to have shapes fitted to the U-Net structure and avoid losing random years

        target_name = self.root_dir+pair[0]+'/'+pair[2] # Path of the data for the second member of the pair            
        target_member = np.load(target_name, allow_pickle=True)[3:,:,:] # Erase the first three years to have shapes fitted to the U-Net structure and avoid losing random years
...
class Model():
    ...
    def __init__(self, variable, model, normalisation, nb_epochs, batch_size, save_path, n_folds):
        ...
        self.root_dir = '/scratch/globc/techer/ia/data_norm/'+self.normalisation+'/'+self.variable+'/'
    ...
    def main(self):
        ...
        np.save(str(model_save_path / 'model_test_fold_'), test_models_dico) # Save test model names for all the folds
...
save_path = Path("/scratch/globc/techer/ia/models/UNet_60")  # Saving path of the trained model
